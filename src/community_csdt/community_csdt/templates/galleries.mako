...
<%block name="title">
  <title>CSDT Community | Galleries</title>
</%block>
    ...
    <script type="text/javascript"> 
      function jqinvoke() {
        jQuery("#class_table").jqGrid({
          url: '${path_url}/all-tables',
          datatype: "json",
          mtype: 'GET',
          colNames:['CID', 'Classname', '# Students', 'Admin Username', 'Admin First Name', 'Admin Last Name', 'Admin Name', 'View Class'],
          colModel:[
            {name:'cid', index:'cid', width:25, sorttype:"int", hidden:true},
            {name:'classname', index:'classname', width:150, sorttype:"string"},
            {name:'size', index:'size', width:150, sorttype:"int"},
